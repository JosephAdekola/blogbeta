...
export default function BlogItem({ image, author, title, content, id, token, warningDisplay, targetID, identity }) {
  return (
    <div>
        ...
        <div key={id} className=' bg-gray-700  rounded overflow-hidden '>
            ...
            <div className=' p-5 flex flex-col gap-5 '>
                ...
                <p className=' text-white font-extralight ' > 
                    {content}... <Link to={`/news_detail/${id}`} className='text-blue-400'>
                    ...
                    <div className='flex gap-2'>
                        <div>
                            <button className={token? 'p-3 bg-gray-500 rounded hover:bg-gray-600': 'hidden' }> 
                                <Link to={`/news_detail/edit/${id}`}>
                                    Edit
                                </Link></button>
                        </div>
                        <button className={token ? 'p-3 bg-gray-500 rounded hover:bg-gray-600' : 'hidden'}
                                onClick={()=>{warningDisplay(true); targetID(identity)}}
                                >
                            Delete
                        </button>
                    </div>
